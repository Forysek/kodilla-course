...
public class BookDirectoryTestSuite {
    ...
    public void listBooksInHandsOfNoBooks() {
        ...
        LibraryDatabase libraryDatabaseMock = mock(LibraryDatabase.class);
        BookLibrary bookLibrary = new BookLibrary( libraryDatabaseMock );
        LibraryUser libraryUser = new LibraryUser("Vito", "Corleone", "64040412345");
        List<Book> resultListOf0Books = new ArrayList<Book>();
        ...
        //Then
        Assert.assertTrue(rentedBooks0.size() == resultListOf0Books.size());
    ...
    public void listBooksInHandsOfOneBook() {
        ...
        BookLibrary bookLibrary = new BookLibrary( libraryDatabaseMock );
        LibraryUser libraryUser = new LibraryUser("Vito", "Corleone", "64040412345");
        Book book = new Book("Random title", "Random Author", 1991);
        List<Book> resultListOf1Books = generateListOfNBooks( 1 );
        when(bookLibrary.listBooksInHandsOf( libraryUser )).thenReturn(resultListOf1Books);

        //When
        List<Book> rentedBooks1 = bookLibrary.listBooksInHandsOf( libraryUser );

        //Then
        Assert.assertEquals(rentedBooks1.size(), resultListOf1Books.size());
    }
    ...
    public void listBooksInHandsOfFiveBooks() {
        ...
        when(bookLibrary.listBooksInHandsOf( libraryUser )).thenReturn(resultListOf5Books);

        //When
        List<Book> rentedBooks5 = bookLibrary.listBooksInHandsOf( libraryUser );

        //Then
        Assert.assertEquals( rentedBooks5.size(), resultListOf5Books.size() );
    }
}