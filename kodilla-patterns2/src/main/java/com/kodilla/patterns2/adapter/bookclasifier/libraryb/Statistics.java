...
public class Statistics implements BookStatistics {
    @Override
    public int averagePublicationYear(Map<BookSignature, BookLibB> books) {
        if(books.size() == 0) return 0;
        int sum = 0;
        for(Map.Entry<BookSignature, BookLibB> entry : books.entrySet()) {
    ...
    @Override
    public int medianPublicationYear(Map<BookSignature, BookLibB> books) {
        if(books.size() == 0) return 0;
